...
strtypemem(L4_Word_t type) {
	static const char *types[] = { "Undefined", // L4_UndefinedMemoryType		(0x0)
...
static int memdesc_info(L4_KernelInterfacePage_t *kip, int pos,
...
void sos_print_error(L4_Word_t ec) {
	L4_Word_t e, p;
...
void sos_print_fpage(L4_Fpage_t fpage) {
	sos_logf("fpage(%lx - %lx)", L4_Address(fpage), L4_Address(fpage)
			+ L4_Size(fpage));
}
...
// Create a new thread
static inline L4_ThreadId_t create_thread(L4_ThreadId_t tid,
	...
	int res = L4_ThreadControl(tid, root_thread_g, // address space
			scheduler, // scheduler
			root_thread_g, // pager
...
L4_ThreadId_t sos_thread_new_priority(L4_Word_t prio, void *entry, void *stack) {
