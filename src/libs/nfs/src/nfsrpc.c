...
void nfs_getattr_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	int err = 0, status = -1;
	fattr_t pattrs;
	...
	if (err == 0) {
		...
		if (status == NFS_OK) {
			...
			getfrombuf(pbuf, (void*) &pattrs, sizeof(fattr_t));
		}
	}
...
int nfs_getattr(struct cookie *fh, void(*func)(uintptr_t, int, fattr_t *),
		uintptr_t token) {
	struct pbuf *pbuf;
...
void nfs_lookup_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	int err = 0, status = -1;
	...
	if (err == 0) {
		...
		if (status == NFS_OK) {
			/* it worked, so take out the return stuff! */
			getfrombuf(pbuf, (void*) &new_fh, sizeof(struct cookie));
			getfrombuf(pbuf, (void*) &pattrs, sizeof(fattr_t));
		}
	}
...
int nfs_lookup(struct cookie *cwd, char *name, void(*func)(uintptr_t, int,
		struct cookie *, fattr_t *), uintptr_t token) {
	struct pbuf *pbuf;
...
void nfs_read_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	int err = 0, status = -1;
	fattr_t pattrs;
	char *data = NULL;
	int size = 0;
	...
	if (err == 0) {
		...
		if (status == NFS_OK) {
			/* it worked, so take out the return stuff! */
			getfrombuf(pbuf, (void*) &pattrs, sizeof(fattr_t));
			getfrombuf(pbuf, (void*) &size, sizeof(int));
...
int nfs_read(struct cookie *fh, int pos, int count, void(*func)(uintptr_t, int,
		fattr_t *, int, char *), uintptr_t token) {
	struct pbuf *pbuf;
	readargs_t args;
...
void nfs_write_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	int err = 0, status = -1;
	fattr_t pattrs;
	void (*cb)(uintptr_t, int, fattr_t *) = callback;
...
int nfs_write(struct cookie *fh, int offset, int count, void *data,
		void(*func)(uintptr_t, int, fattr_t *), uintptr_t token) {
	struct pbuf *pbuf;
	writeargs_t args;
...
void nfs_create_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	int err = 0, status = -1;
	struct cookie new_fh;
	fattr_t pattrs;
	void (*cb)(uintptr_t, int, struct cookie *, fattr_t *) = callback;
...
int nfs_create(struct cookie *fh, char *name, sattr_t *sat, void(*func)(
		uintptr_t, int, struct cookie *, fattr_t *), uintptr_t token) {
	struct pbuf *pbuf;
...
int getentries_readdir(struct pbuf *pbuf, int *cookie) {
...
void nfs_readdir_cb(void * callback, uintptr_t token, struct pbuf *pbuf) {
	...
	if (err == 0) {
		...
		if (status == NFS_OK) {
			...
			entries = malloc(sizeof(struct nfs_filename) * num_entries);
			...
			while (tmp) {
				...
				getfrombuf(pbuf, (char*) &fileid, sizeof(fileid));
				debug("Got filed: %d\n", fileid);
				getfrombuf(pbuf, (char*) &entries[count].size, sizeof(int));
				debug("Got size: %d\n", entries[count].size);
...
int nfs_readdir(struct cookie *pfh, int cookie, int size, void(*func)(
		uintptr_t, int, int, struct nfs_filename *, int), uintptr_t token) {
	readdirargs_t args;
